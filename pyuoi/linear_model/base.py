    ...
    def _pre_fit(self, X, y):
        ...
        if self.standardize:
            if self.fit_intercept and issparse(X):
                msg = "Cannot center sparse matrices: " "pass `fit_intercept=False`"
    ...
    def fit(self, X, y, stratify=None, verbose=False):
        ...
        if size > self.n_boots_sel:
            tasks = np.array_split(
                np.arange(self.n_boots_sel * self.n_reg_params_), size
            )[rank]
            selection_coefs = np.empty((tasks.size, n_coef))
            my_boots = dict(
                (task_idx // self.n_reg_params_, None) for task_idx in tasks
            )
        else:
            # split up bootstraps into processes
            tasks = np.array_split(np.arange(self.n_boots_sel), size)[rank]
        ...
        for boot in range(self.n_boots_sel):
            if size > 1:
                if rank == 0:
                    ...
                    rvals = [None] * 2
                rvals = [Bcast_from_root(rval, self.comm, root=0) for rval in rvals]
        ...
        for ii, task_idx in enumerate(tasks):
            ...
            if curr_boot_idx != boot_idx:
                if hasattr(self._selection_lm, "coef_"):
                    self._selection_lm.coef_ *= 0.0
                if hasattr(self._selection_lm, "intercept_"):
                    self._selection_lm.intercept_ *= 0.0
            ...
            if size > self.n_boots_sel:
                msg = "selection bootstrap %d, " "regularization parameter set %d" % (
                    boot_idx,
                    reg_idx,
                )
        ...
        if size > 1:
            ...
            if rank == 0:
                ...
                supports = self.intersect(
                    selection_coefs, self.selection_thresholds_
                ).astype(int)
            else:
                supports = None
        ...
        #####################
        # set up data arrays
        tasks = np.array_split(np.arange(self.n_boots_est * self.n_supports_), size)[
            rank
        ]
        my_boots = dict((task_idx // self.n_supports_, None) for task_idx in tasks)
        ...
        for ii, task_idx in enumerate(tasks):
            ...
            if np.any(support):
                ...
                if self.shared_support:
                    ...
                    estimates[
                        ii, np.tile(support, self.output_dim)
        ...
        if size > 1:
            estimates = Gatherv_rows(send=estimates, comm=self.comm, root=0)
            scores = Gatherv_rows(send=scores, comm=self.comm, root=0)
            ...
            if rank == 0:
                estimates = estimates.reshape(
                    self.n_boots_est, self.n_supports_, n_coef
                )
                ...
                coef = np.median(best_estimates, axis=0).reshape(
            ...
            self.intercept_ = Bcast_from_root(self.intercept_, self.comm, root=0)
            self.rp_max_idx_ = self.comm.bcast(self.rp_max_idx_, root=0)
        else:
            self.estimates_ = estimates.reshape(
                self.n_boots_est, self.n_supports_, n_coef
            )
            ...
            best_estimates = self.estimates_[
                np.arange(self.n_boots_est), self.rp_max_idx_, :
            ]
            # take the median across estimates for the final, bagged estimate
            self.coef_ = np.median(best_estimates, axis=0).reshape(
                self.output_dim, n_features
            )
            self._fit_intercept(X, y)
        self._post_fit(X, y)
...
class AbstractUoILinearRegressor(AbstractUoILinearModel, metaclass=_abc.ABCMeta):
    ...
    def __init__(
        ...
        if estimation_target is not None:
            if estimation_target not in ["train", "test"]:
                raise ValueError("invalid estimation target: %s" % estimation_target)
            else:
    ...
    def _pre_fit(self, X, y):
        ...
        if y.ndim == 1:
            ...
            if y.shape[1] > 1:
                raise ValueError(
                    "y should either have shape " + "(n_samples, ) or (n_samples, 1)."
                )
        else:
            raise ValueError(
                "y should either have shape " + "(n_samples, ) or (n_samples, 1)."
    ...
    def _post_fit(self, X, y):
        ...
        if self.standardize:
            ...
            if self.fit_intercept:
                ...
                self.intercept_ += sy.mean_ - np.dot(sX.mean_, self.coef_.T)
    ...
    def _score_predictions(self, metric, fitter, X, y, support, boot_idxs):
        ...
        if metric == "r2":
            ...
            ll = utils.log_likelihood_glm(model="normal", y_true=y, y_pred=y_pred)
    ...
    def _fit_intercept(self, X, y):
        """Fit the intercept."""
        if self.fit_intercept:
            self.intercept_ = y.mean(axis=0) - np.dot(X.mean(axis=0), self.coef_.T)
        else:
...
class AbstractUoIGeneralizedLinearRegressor(
    ...
    _default_est_targets = {"log": 1, "AIC": 0, "AICc": 0, "BIC": 0, "acc": 1}
    ...
    def _post_fit(self, X, y):
        ...
        if self.standardize and self.fit_intercept:
            sX = self._X_scaler
            self.intercept_ += np.dot(sX.mean_ * sX.scale_, self.coef_.T)
