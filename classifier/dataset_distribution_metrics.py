...
class Dataset:
    ...
    def initialize_dataset(self, dataset):
        self.dataset = xr.load_dataset(dataset, engine="h5netcdf").to_dataframe().value_counts()

    @dispatch(pd.DataFrame)
    def initialize_dataset(self, dataset):
        """
        The dataframe must have a behavior name column and the behavior names must be unique.
        """
        self.dataset = dataset
        assert dataset.behavior_name.is_unique

    def distribution(self, output_file: Optional[str] = None, plot: bool = False) -> dict[str, int]:
        """
        ...
        counts = {behavior: count for behavior, count in zip(self.dataset["behavior_name"], self.dataset["count"])}
