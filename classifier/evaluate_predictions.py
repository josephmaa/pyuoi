...
def initialize_arg_parser():
    ...
    parser.add_argument("--log_summary_output_file", help="Output model metrics to log file. Includes precision, recall, F1 score, auc-roc score.", type=str)
    return parser
...
def read_numpy_binary_array(attributes: dict, key: str, dtype: np.dtype) -> np.array:
    ...
    base64_array = base64.b64decode(attributes[key][1])
    array = np.frombuffer(base64_array, dtype=dtype).reshape(shape)
    return array


def graph_2d_subset_x_linear_classification_coefficients(
    key: str,
    data: NpzFile,
    frame_start: int = 0,
    frame_end: int = 10,
    ...
    with np.printoptions(precision=2, suppress=True):
        ...
        plot = ax.imshow(
            coef, interpolation="none", extent=[0, 24, 24, 0], vmin=-15, vmax=15
        )
...
def main(parsed_args: argparse.Namespace, debug: bool = False):
    ...
    >>> python classifier/evaluate_predictions.py --input_file /Users/josephgmaa/pyuoi/pyuoi/data/features/run_parameters/20220712-142558.run_parameters.json --key="supports_"

    >>> python classifier/evaluate_predictions.py --input_file /Users/josephgmaa/pyuoi/pyuoi/data/features/run_parameters/20220712-142558.run_parameters.json --key="accuracy"

    >>> python classifier/evaluate_predictions.py --log_summary_output_file /Users/josephgmaa/pyuoi/pyuoi/data/evaluations
    """
    ...
    if file.endswith(".npy"):
        with np.printoptions(edgeitems=10):
            results = np.load(file=file, allow_pickle=True)
        ...
        with open(file) as data:
            ...
            if key == "supports_":
                ...
                with open(
                    attributes["label_encoder"], "rb"
                ) as label_encoder_pickle_file:
                ...
                (
                    probabilities_x_train,
                    probabilities_x_test,
                    probabilities_y_train,
                ...
                for i in range(n_classes):
                    ...
                    display = PrecisionRecallDisplay.from_predictions(
                        y_test[:, i],
                        probabilities_y_test[:, i],
                        name="PyUOI classifier",
                    )

                    import pandas as pd

                    print(f"y_test: {y_test[:, i]}, {probabilities_y_test[:, i]}")
                    ...
                    df.sort_values(
                        "probabilities_y_test", inplace=True, ascending=False
                    )
                    df.plot(x="y_test", y="probabilities_y_test", kind="bar")
                    plt.title(
