...
def main(parsed_args: argparse.Namespace):
    ...
    >>> python classifier/rat7m_classifier.py --input_files ~/features/PCs/PCs-mavg-velocity_relative.netcdf --column_names PCA_mavg_velocity_relative_0
    ...
    if parsed_args.use_small_dataset:
        ...
        pc_netcdfs = [
            file
            for file in absolute_file_paths(parsed_args.grid_search)
            if file.endswith(".netcdf")
        ]
        for files in combinations(pc_netcdfs, 3):
            df = pd.DataFrame()
            for i, filename in enumerate(files):
                dataset = xr.load_dataset(filename, engine="h5netcdf").to_dataframe()
                if i == 0:
            ...
            x_train, x_test, y_train, y_test = train_test_split(
                df.loc[:, df.columns != "behavior_name"].to_numpy(),
                le.transform(y),
                random_state=parsed_args.training_seed,
            )

            l1log = UoI_L1Logistic(
                random_state=parsed_args.model_seed, multi_class="multinomial"
            ...
            le_filename = generate_timestamp_filename(
                ...
                basename="label_encoder",
                file_format=".pkl",
            )
            with open(le_filename, "wb+") as f:
                pickle.dump(le, f)
            print(f"Label encoder written to {le_filename}.")
            ...
            dump_json(
                ...
                results={
                    ...
                    "jensenshannon_predictions": jensenshannon_predictions,
                    "prediction_probabilities": l1log.predict_proba(x_test),
        ...
        x_train, x_test, y_train, y_test = train_test_split(
            df.loc[:, column_indices].to_numpy(),
            le.transform(y),
            random_state=parsed_args.training_seed,
        )
    ...
    assert (
        x_train.shape[1] > 0
    ), f"X train dataset should have at least 1 input feature. Try checking that the column names match the input dataset: {list(df.columns)}"
